...
void CL_KeyDownEvent( int key, unsigned time )
{
	...
	if ( cl_altTab->integer )
	{
		if( keys[K_COMMAND].down ) // || keys[K_COMMAND].binding || keys[K_COMMAND].repeats )
		{
			// window mode
			if ( key == K_ENTER ) {
			...
			// console
			if ( key == '`' || key == '~' || key == 0x7e || key == 0x60 || key == K_ESCAPE ) {	
				Con_ToggleConsole_f ();
				Key_ClearStates ();
				return;
			}
...
#ifndef WIN32
	// if alt-tab key is enabled, minimize when pressed
