...
void Con_ToggleConsole_f (void) {
	...
	// Hack to fix some weird problem with a single blank space
	for ( c = g_consoleField.buffer; *c; c++ )
	{
		if ( *c != ' ' )
		{
			only_blanks = qfalse;
...
void Con_DrawNotify (void)
{
	...
	if( Key_GetCatcher( ) & KEYCATCH_MESSAGE )
	{
		...
		if( chat_team )
		{
			SCR_DrawBigString( 8, 0, "Team Say:", 1.0f, qfalse );
			skip = 11;
		}
		else if( chat_admins )
		{
			SCR_DrawBigString( 8, 0, "Admin Say:", 1.0f, qfalse );
			skip = 11;
		}
		else if (prompt.active)
		{ 
			SCR_DrawBigString( 8, 0, prompt.question, 1.0f, qfalse );
			skip = strlen(prompt.question) + 1;	
		}
		else if( chat_clans )
		{
			SCR_DrawBigString( 8, 0, "Clan Say:", 1.0f, qfalse );
			skip = 11;
		}
		else
		{ 
			SCR_DrawBigString( 8, 0, "Say:", 1.0f, qfalse );
			skip = 5;
		}

		Field_BigDraw( &chatField, skip * BIGCHAR_WIDTH, 0, qtrue, qtrue );
	}
}
