...
void CL_FirstDownload(void) {
	...
	if( cl_allowDownload->integer & DLF_ENABLE ) {
		...
		if( s ) {
			...
			if( s ) memmove( clc.downloadList, s, strlen(s) + 1);
		}
		else {
...
void CL_NextDownload(void) {
	...
	if (*clc.downloadList) {
		...
		if ( ! ( allowautodl ) ) {
			Cvar_Set("com_errorMessage", va("To play on this server, "
				"you need the map:\n\n%s.\n\n"
				"If you trust data from this server and want to try"
				"to automatically download the map, press ANY key.\n\n"
...
Show the download menu and handle the key
=================
*/
...
void CL_CheckTeamChange(void)
{
	...
	if( previousteam != team )
	{
		...
		{
			if( *cl_blueConfig->string )
			{
				Cbuf_AddText( va( "exec %s\n", cl_blueConfig->string ) );
			}
		}
	} 
}
...
void CL_Init( void ) {
	...
	Cvar_Alias (cl_allowDownload, "cl_autoDownload");
	...
	cl_redConfig = Cvar_Get ("cl_redConfig", "", CVAR_ARCHIVE);
	cl_blueConfig = Cvar_Get ("cl_blueConfig", "", CVAR_ARCHIVE);
	...
	Cvar_Get ("g_redTeam", "Red Dragons", CVAR_SERVERINFO | CVAR_ARCHIVE);
	Cvar_Get ("g_blueTeam", "Blue Swat", CVAR_SERVERINFO | CVAR_ARCHIVE);
