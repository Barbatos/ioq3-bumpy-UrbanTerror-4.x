...
static void makeFloat(int depth)
{
	...
	if(opStackRegType[depth] == 1)
	{
		...
		if(opStackLoadInstructionAddr[depth])
		{
			...
			{
				//COM_Printf("patching LWZ at %p to LFS at depth %ld\n",opStackLoadInstructionAddr[depth],depth);
				//COM_Printf("old instruction: %08lx\n",instruction);
				// Extract registers
				...
				//COM_Printf("old dest: %ld\n",destReg);
				destReg = (destReg - opStackIntRegisters[0]) + opStackFloatRegisters[0];
				instruction = PPC_LFS | ( destReg << 21 ) | ( aReg << 16 ) | imm ;			
				//COM_Printf("new dest: %ld\n",destReg);
				//COM_Printf("new instruction: %08lx\n",instruction);
			}
			else
			{
				//COM_Printf("patching LWZX at %p to LFSX at depth %ld\n",opStackLoadInstructionAddr[depth],depth);
				//COM_Printf("old instruction: %08lx\n",instruction);
				// Extract registers
				destReg = (instruction >> 21) & 31;
				aReg    = (instruction >> 16) & 31;
		...
		{	
			//COM_Printf("doing float constant load at %p for depth %ld\n",&buf[compiledOfs],depth);
...
//                InstImm( "bc", PPC_BC, 4, 0, v );
    ...
    //            Inst( PPC_FRSP, R_TOP, 0, R_TOP );
		...
		#if DEBUG_VM
		if(pass == 1)
		COM_Printf("%08x BLOCK_COPY\t%08lx\n",instruction,v<<2);
		#endif
...
int	VM_CallCompiled( vm_t *vm, int *args ) {
	...
	//COM_Printf("VM_CallCompiled: %p   %08lx %08lx %08lx\n",
